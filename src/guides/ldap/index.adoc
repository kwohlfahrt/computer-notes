...
LDAP (Lightweight Directory Access Protocol) is a protocol to share directory
information (e.g. names, e-mail addresses, etc). The
https://www.openldap.org/[OpenLDAP] project maintains an open-source LDAP server
and client.
...
show all attributes of the entry, but not any of its children.
...
users, this would be `ldapsearch -b ou=people,dc=example,dc=com`.
...
Alternatively, the `changetype: add` can be omitted `ldapadd` or `ldapmodify -a`
is used.
